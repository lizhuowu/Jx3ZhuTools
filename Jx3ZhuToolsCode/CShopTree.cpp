...
CCShopTree::CCShopTree(void)
{
	...
	if(SHOP_ALL_BASE && IsReadAddrInt((PVOID)SHOP_ALL_BASE))
	{
		...
		BASE_OFFSET = *(int *)(BASE_VALUE + GET_SHOP_ADDR_BY_SHOPID_TREE_BIG_OFFSET + TREE_SHOP_SMALL_OFFSET);
	...
	__CATCH("初始化所有商店二叉对象出错")
}
//遍历二叉树
void CCShopTree::TreeList(int StartAddr)
{
